...
from modules import classification_network
...
class RecurrentAttention(nn.Module):
    ...
    def __init__(self,
        ...
        # image_size = g*g*c*lambda, where lambda = 2 in our case
        self.classifier = action_network(hidden_size, image_size)
    ...
    def forward(self, x, l_t_prev, h_t_prev, last=False):
        ...
        """
        g_t, old_phi = self.sensor(x, l_t_prev)
        ...
        image_formed = self.classifier(h_t)

        # if last:
        #     # log_probas = self.classifier(h_t)
        #     image_formed = self.classifier(h_t)
        #     # return h_t, l_t, b_t, log_probas, log_pi
        #     return h_t, l_t, b_t, image_formed, log_pi

        # return h_t, l_t, b_t, image_formed, log_pi
        return h_t, l_t, b_t, image_formed, log_pi, old_phi
        # return h_t, l_t, b_t, log_pi


class RecurrentAttentionClassificaton(nn.Module):
    ...
    def __init__(self,
        ...
        - num_glimpses: number of glimpses to take per image,
          i.e. number of BPTT steps.
        """
        super(RecurrentAttentionClassificaton, self).__init__()
        self.std = std

        self.sensor = glimpse_network(h_g, h_l, g, k, s, c)
        self.rnn = core_network(hidden_size, hidden_size)
        self.locator = location_network(hidden_size, 2, std)
        self.classifierNew = classification_network(hidden_size, num_classes)
        # image_size = g*g*c*lambda, where lambda = 2 in our case
        # self.classifier = action_network(hidden_size, image_size)
        # self.baseliner = baseline_network(hidden_size, 1)
    ...
    def forward(self, x, l_t_prev, h_t_prev, last=False):
        ...
        # log_pi = torch.sum(log_pi, dim=1)
        if last:
          log_probas = self.classifierNew(h_t)
          return h_t, l_t, log_probas
