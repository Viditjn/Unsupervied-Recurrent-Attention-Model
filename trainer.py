        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for t in range(self.num_glimpses):
                    ...
                    h_t, l_t, b_t, new_img_patch, p, old_phi = self.model(x, l_t, h_t)
                    ...
                    log_pi.append(p)
                    glimpses_patch.append(old_phi[0:9])
                ...
                pbar.set_description(
                    (
                        "{:.1f}s - loss: {:.3f} - R: {:.3f} - reinforce_r: {:.3f}".format(
                            (toc-tic), loss.data, R, loss_reinforce
                        )
                    )
                )
                ...
                if plot:
                    if self.use_gpu:
                        ...
                        glimpses_patch = [gp.cpu().data.numpy().squeeze() for gp in glimpses_patch]
                    else:
                        ...
                        glimpses_patch = [gp.data.numpy().squeeze() for gp in glimpses_patch]
                    pickle.dump(
                        imgs, open(
                    ...
                    pickle.dump(
                        glimpses_patch, open(
                            self.plot_dir + "gp_{}.p".format(epoch+1),
                            "wb"
                        )
                    )
