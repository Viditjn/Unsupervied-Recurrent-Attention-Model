...
class retina(object):
    ...
    def foveate(self, x, l):
        ...
        def check_glimpses():
            ...
            def showAll(i) :
                fig, axs = plt.subplots(nrows=1, ncols=len(phi)+1)
                im1 = np.transpose(x[i], (1,2,0))
                color = 'r'
                for j, ax in enumerate(axs.flat):
                    if j > 0:
                        im = np.transpose(phi[j-1][i], (1,2,0))
                        ax.imshow(im)
        ...
        # pdb.set_trace()
        phi_old = phi
        phi_old = torch.cat(phi_old, 1)
        phi_old = phi_old.view(phi_old.shape[0], -1)
...
class action_network(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        # doubt whether to output entire image or just a patch? If entire image then
        # its not solving the problem of complexity. If small patch then how to predict
        # entire image, it will only predict the patches being used and in that case we
        # calulate the reward at that time only.
        # A possible solution is to output some bigger patch at the location being looked
        # then combine them all.
        self.fc = nn.Linear(input_size, output_size)
        # output_size equal to some patch size bigger than the original patch
...
class classification_network(nn.Module):
    ...
    def forward(self, h_t):
        h1 = self.i2h(h_t)
        h2 = self.h2h(h1)
        a_t = F.log_softmax(self.fc(h2), dim=1)
        return a_t
